...
def runBySection(PDF_FILENAME_PREFIX, section_list, is_cumulative, is_percent, arrayFiles, questions_data, responses_data):
    for i, sec in enumerate(section_list):
        ...
        section_filename = pdf_name_generator.generate_pdf_name(PDF_FILENAME_PREFIX, i+1)
        
        cities, pvals = graphing.make_subplots_each_question(
                            section_iterator, arrayFiles, section_filename, is_cumulative, is_percent, questions_data, responses_data
                            )
    return pvals